<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/jquery/jquery-2.2.3.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/angularjs/angular.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/angularjs/angular-messages.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/angularjs/angular-validation-match.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/bootstrap/js/bootstrap.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/blockui/blockui.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/alertifyjs/alertify.min.js"></script>
<script type="text/javascript"
        src="${pageContext.request.contextPath}/plugins/jquery.formstyler/jquery.formstyler.min.js"></script>
<script src="${pageContext.request.contextPath}/static/utility.js"></script>